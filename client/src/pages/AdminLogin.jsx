...
export default function AdminLogin() {
  ...
  return (
    <div style={{ minHeight: "100vh", background: theme.INK, display: "flex", alignItems: "center", justifyContent: "center", padding: 24 }}>
      <div style={{ background: theme.CREAM, width: "100%", maxWidth: 420, padding: 48 }}>
        ...
        <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: 20 }}>
          <div>
            ...
            <div style={{ position: "relative" }}>
              ...
              <input required type="email" style={fieldStyle} placeholder="admin@myspacehotels.com" value={form.email} onChange={e => setForm({ ...form, email: e.target.value })} />
            </div>
          </div>
          <div>
            ...
            <div style={{ position: "relative" }}>
              ...
              <input required type="password" style={fieldStyle} placeholder="••••••••" value={form.password} onChange={e => setForm({ ...form, password: e.target.value })} />
            </div>
          </div>
          {error && <div style={{ color: "#a33", fontSize: 13, padding: "12px 16px", background: "#fff5f5", border: "1px solid #fcc" }}>{error}</div>}
          <button type="submit" disabled={loading} className="cta-btn" style={{ background: theme.SEA, color: theme.CREAM, border: "none", padding: 16, fontSize: 13, letterSpacing: "0.15em", textTransform: "uppercase", fontWeight: 500, display: "flex", alignItems: "center", justifyContent: "center", gap: 10, opacity: loading ? 0.7 : 1, cursor: loading ? "not-allowed" : "pointer" }}>
        ...
        <p style={{ fontSize: 12, color: theme.MUTED, marginTop: 24, textAlign: "center" }}>
          Not an admin? <a href="/" style={{ color: theme.SEA_DARK }}>Back to site →</a>
        </p>
      </div>
    </div>
  );
