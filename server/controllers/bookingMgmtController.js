...
import { isRazorpayRefundEligible, commitBookkeepingRefund, processRazorpayRefund } from "../services/refundService.js";
...
export const refund = async (req, res) => {
  try {
    ...
    if (!isRazorpayRefundEligible(booking)) {
    ...
    const result = await processRazorpayRefund({
      ...
      amount, reference, reason, actorId: req.user.id,
    });

    if (result.outcome === "duplicate") return res.status(409).json({ message: result.message });
    if (result.outcome === "invalid") return res.status(400).json({ message: result.message });
    if (result.outcome === "gateway_failed") return res.status(502).json({ message: result.message });
    if (result.outcome === "reconciliation_required") {
      return res.status(500).json({ message: result.message, razorpay_refund_id: result.razorpay_refund_id });
    }

    await audit({ userId: req.user.id, userEmail: req.user.email, action: "refund", entityType: "booking", entityId: id, beforeData: booking, afterData: result.booking, metadata: { amount, reference, reason, razorpay_refund_id: result.razorpay_refund_id } });
    res.json({ message: "Refund processed via Razorpay", booking: result.booking, razorpay_refund_id: result.razorpay_refund_id });
  } catch (e) { res.status(500).json({ message: e.message }); }
};
